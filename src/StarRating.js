            ...
            <div style={starContainerStyle}>
                {Array.from({ length: maxRating }, (_, i) => (
                    <Star onRate={() => handleRating(i + 1)}
                        ...
                        full={tempRating ? tempRating >= i + 1 : rating >= i + 1} color={color} size={size} key={i} />
                ))}
            </div>
            <p style={textStyle}>{rateMessage.length === maxRating ? rateMessage[tempRating ? tempRating - 1 : rating - 1] : tempRating || rating || ""}</p>
        </div>
    )
}
